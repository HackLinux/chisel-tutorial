...
class RiscWrapperTests(c: RiscWrapper) extends DaisyWrapperTester(c, false) {
  ...
  def I (op: UInt, rc: Int, ra: Int, rb: Int) = 
    Cat(op, UInt(rc, 8), UInt(ra, 8), UInt(rb, 8))
  val app  = Array(
    I(c.top.imm_op,   1, 0, 1), // r1 <- 1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   1, 0, 2), // r1 <- 2
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   1, 0, 3), // r1 <- 3
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   1, 0, 4), // r1 <- 4
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   1, 0, 5), // r1 <- 5
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   1, 0, 6), // r1 <- 6
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   1, 0, 7), // r1 <- 7
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   1, 0, 8), // r1 <- 8
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   1, 0, 9), // r1 <- 9
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   1, 0, 10), // r1 <- 10
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.add_op,   1, 1, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 1), // r1 <- 1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 2), // r1 <- 2
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 3), // r1 <- 3
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 4), // r1 <- 4
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 5), // r1 <- 5
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 6), // r1 <- 6
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 7), // r1 <- 7
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 8), // r1 <- 8
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 9), // r1 <- 9
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.imm_op,   2, 0, 10), // r1 <- 10
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
    I(c.top.add_op,   2, 2, 1), // r1 <- r1 + r1
  ...
  do {
    tick(); k += 1
  } while (peek(c.top.io.valid) == 0 && k < 400)
  expect(k < 400, "TIME LIMIT")
  expect(c.top.io.out, 40)
}
