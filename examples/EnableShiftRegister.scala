...
// same as EnableShiftRegisterTests but extends DaisyTester
class EnableShiftRegisterDaisyTests(c: EnableShiftRegister) extends DaisyTester(c, false) {  
  val reg = Array.fill(4){ 0 }
  for (t <- 0 until 200) {
    val in    = rnd.nextInt(2)
    val shift = rnd.nextInt(2)
...
class EnableShiftRegisterWrapperTests(c: EnableShiftRegisterWrapper) extends DaisyWrapperTester(c, false) {
  val reg = Array.fill(4){ 0 }
  for (t <- 0 until 200) {
    val in    = rnd.nextInt(2)
    val shift = rnd.nextInt(2)
